---
layout: post
title: [Paper review] Recurrent Glimpse-based Decoder for Detection with Transformer
...
DETR의 output 또는 이전 block의 출력값중 $O_{box}(i-1)$을 이용하여 해당영역에서 feature를 추출하게 된다. 이때에 box 값을 그대로 사용하는 것이 아니라 margin을 갖고 조금더 넓은 영역에서 feature를 추출하는데 Diagram에서 Enlarge라고 표시된 부분이다. 이때 enlarge 하는 비율을 나타내는 $\alpha$ 값은 점점 감소하게 되는데 3step일 경우를 예로 들면 $\alpha=3,2,1$ 이된다.
...
<div class="row mt-3">
    ...
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.html path="assets/img/rego05.png.jpg" class="img-fluid rounded z-depth-1" %}
    </div>
</div>
<div class="caption">
